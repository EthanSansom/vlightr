...
a vector to style, colour, or otherwise re-format it's elements (if they meet
some predicate) when the vector is printed or formatted. Highlights are persistent, 
meaning highlighted vectors can be manipulated (with minimal legwork) while 
maintaining their custom formatting.
...
Highlighted vectors can't be coerced or cast to other vector types. To use a
function which expect the highlight's underlying vector type, first 
`un_highlight()/ul()` to expose the highlighted data and then `re_highlight()/rl()` 
to re-apply the conditional formatting.
