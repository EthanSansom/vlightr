...
For it's basic functionality, the `highlight()` function requires 3
...
The `precedence` of highlighted vectors is maintained when they are combined. For
example, you can reduce highlighted vector's `precedence` to prioritize it's
format.
...
summary_hl <- highlight_case(
  ...
  .x == quantile(.x, 0.5) ~ paste0("(Mode = ", .x, ")"),
...
-   `x` is not a dataframe (or dataframe subclass)
