...
Including observations in a dataframe or tibble.

```{r, collapse=TRUE}
tibble::tibble(x = above_mean[1:5])
```

Highlights are generic, meaning that many vector's you'll encounter in
...
`describe_highlight()` is useful to understand how a highlight is
formatted.
...
combined. For example, you can reduce highlighted vector's `precedence`
to prioritize it's format.

```{r}
even_is_red <- hl(1:3, ~ .x %% 2 == 0, cli::col_red, precedence = 1)
even_is_green <- hl(4:6, ~ .x %% 2 == 0, cli::col_green, precedence = 0)
...
-   `style()` returns a `cli::style_*` function which bold, underline,
    or italicize text among other things
...
The `highlight()` function (and friends `hl()`, `highlight_case()`) provide some
extra arguments which are useful for refining how user-generated vectors are
highlighted.
...
just that. While we're at it, we'll replace the repeated timezone with a single
timezone placed after each element.
...
format_interval <- function(x) {
  ...
  # Format the output like "start--end tzone"
  out <- paste0(start, "--", end, " ", attr(x, "tzone"))
  
  # Remember to preserve NA values in the formatted output
...
We'll package the `init_formatter` and `last_formatter` together in a highlighter,
so we can use them whenever we want to highlight an interval vector.
